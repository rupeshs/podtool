...
const defs: Record<string, string> = {
  ...
  stop: wrap('<rect x="5" y="5" width="14" height="14" rx="1.5"/>'),
  rotate: wrap(
...
export function icon(name: keyof typeof defs): string {
  return defs[name] ?? "";
}

export function mountIcons(root: ParentNode = document): void {
  root.querySelectorAll<HTMLElement>("[data-icon]").forEach((el) => {
    const name = el.dataset.icon as keyof typeof defs;
    if (defs[name]) el.innerHTML = defs[name];
  });
}
